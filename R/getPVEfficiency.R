...
#' @return the original \code{data.frame} augmented by a few columns:\itemize{
#'   \item \code{tiltedCorrection} a correction factor to convert sun irradiance on earth surface to irradiance on tilted PV panels
#'  \item \code{tiltedPanelG} the actual irradiance on the tilted panels in \eqn{W/m^2}
#'  \item \code{Tc} temperature of the panel in \eqn{^\circ}C
#'  \item \code{tiltedTc} temperature of the panel corrected for panel tilt in \eqn{^\circ}C
#'  \item \code{eta} PV panel efficiency in \eqn{\%}
#'  \item \code{tiltedEta} PV panel efficiency corrected for panel tilt in \eqn{\%}
#'  \item \code{electricPower} Potential electric power in \eqn{W/m^2} generated by PV cells
#'  \item \code{tiltedEPower} Potential electric power in \eqn{W/m^2} generated by tilted PV cells
#'  \item \code{lossesTiltedEPower} Actual electric power in \eqn{W/m^2} generated by tilted PV cells and by taking into account mean percentual losses occurring in the PV system
#'  \item \code{thermalPower} Potential thermal power in \eqn{W/m^2} dispersed by PV cells (basically the whole amount of irradiance that does not get converted into electric power)
#'  \item \code{tiltedTPower} Actual thermal power in \eqn{W/m^2} dispersed by tilted PV cells
#' }
#' 
#' @export
